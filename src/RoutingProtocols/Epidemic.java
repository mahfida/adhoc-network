 ...
 //if nx has packet and ny has to receive it
   ...
   for (Iterator<Map.Entry<Packet,Node>> i = nx.DestNPacket.entrySet().iterator(); i.hasNext(); )
    ...
    Packet packetObj = entry.getKey();
        ...
        if((!ny.packetIDHash.contains(packetObj.packetName))&& packetObj.packetSize<=dtnrouting.contactDuration[nx.ID-1][ny.ID-1]){
        ...
        if((ny.queueSizeLeft>packetObj.packetSize)&&(packetObj.ispacketDelivered==false))
            { 
                ...
                else
                {
                   ...
                   if (ny.name.startsWith("R")||ny.name.startsWith("D"))  deliverPacket(nx,ny,destNode,packetObj); }
...
//******************************************************************************
//DELIVER packet TO AN INTERMEDIATE NODE
