...
public class DummyMovement extends dtnrouting
{
    //Instance Variables
    Random rand;
...
public void DatasetMovement()
{
    ...
    for (int k=0;k<nodeArray.size();k++)
    {
        ...
        if((x_reached==true)&&(y_reached==true))
        {
           ...
           {
                isSamePoint(n);
                n.nextX=DSNextPoint(k,'x',n);
                n.nextY=DSNextPoint(k,'y',n);
               // System.out.println(n.name+":"+n.nextX+n.nodeX);
                n.takeNewPosition=false;
                n.nodeRegion.count+=1;
                if(n.nodeRegion.count==n.nodeRegion.numNodes)
                {
